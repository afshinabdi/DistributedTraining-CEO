...
class DeterministicISGQuantizer:
    """
       ...
       mv: First, X and Y are quantized independently according to their expected distribution. Next, the reconstruction points are optimized to minimize the error |G-X'Y|
       mvu: First, X and Y are quantized independently according to their expected distribution. Then, the reconstruction points are optimized such that |G-X'Y| is minimized subject to sum(G-X'Y)=0
    """
    ...
    def quantize(self, X, Y, model=('sfn', 'sn'), num_levels=(2, 2), method='naive', opt_iterations=1):
        ...
        if method == 'mv':
            ...
            for _ in range(opt_iterations):
                X_hat = cX[qX]
                optimize_centers_mviq(A=X_hat.transpose(), B=G, Q=qY, centers=cY,
                                      keep_sparsity=(model[0][0] == 's'))

                Y_hat = cY[qY]
                optimize_centers_mviq(A=Y_hat.transpose(), B=G.transpose(), Q=qX, centers=cX,
                                      keep_sparsity=(model[1][0] == 's'))
            ...
            for _ in range(opt_iterations):
                X_hat = cX[qX]
                optimize_centers_mvuiq(A=X_hat.transpose(), B=G, Q=qY, centers=cY,
                                       keep_sparsity=(model[0][0] == 's'))

                Y_hat = cY[qY]
                optimize_centers_mvuiq(A=Y_hat.transpose(), B=G.transpose(), Q=qX, centers=cX,
                                       keep_sparsity=(model[1][0] == 's'))
    ...
    def _quantize(self, X, model, num_levels):
        ...
        if model in ('uniform', 'u', 'sparse-uniform', 'su'):
            ...
            scale = sla.norm(
                X) / np.sqrt(np.count_nonzero(np.abs(X) > 1e-10) + 1e-12)
...
class DitheredISGQuantizer:
    ...
    def quantize(self, W, num_levels=2, sparse=True, bucket_size=None):
        ...
        if W.size % bucket_size != 0:
            raise ValueError(
                'the number of variables must be divisible by the bucket size.')
