"""
   implementation of the MNIST database
"""
import os
import numpy as np
import gzip
...
def create_dataset(db_dir, batch_size, vector_fmt=False, one_hot=True, validation_size=5000, source_url=DEFAULT_SOURCE_URL):
    """
    ...
    number_samples = {'train': len(train_labels),
...
class MNISTDataset:
    ...
    def _prepare_samples(self, db_dir, vector_fmt, one_hot, validation_size):
        ...
        if not 0 <= validation_size <= len(self._train_images):
            raise ValueError('Validation size should be between 0 and {}. Received: {}.'
                             .format(len(self._train_images), validation_size))
        ...
        self._number_samples = {'train': len(self._train_labels),
