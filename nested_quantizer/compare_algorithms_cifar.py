...
from CifarNet import CifarNetModel
...
output_folder = '../../data/DistributedDataTraining/CEO/CifarNet/NDQSG/'
output_folder = output_folder + training_algorithm + '/exp{}/'
cifar10_settings = {'database-dir': '../../Database/CIFAR10/raw',
                    'one-hot': False,
...
def evaluate_basemodel(nn_params, seed):
    cifar10_db = Cifar10Dataset(db_settings=db_settings)
    cifar10_db.load_from_file()
    db_graph = tf.Graph()
    with db_graph.as_default():
        tf.random.set_random_seed(seed)
        db_images, db_labels, db_initializer = cifar10_db.create_dataset(['train', 'test'], total_batch_size, 16)
...
def evaluate_1bit(num_workers, nn_params, seed):
    ...
    for n in range(max_iterations):
        ...
        for k in range(num_workers):
            ...
            r = np.sum([cmp.compute_entropy(v, 2) for v in q_gW]) + \
                np.sum([cmp.compute_entropy(v, 2) for v in q_gb])
            entropy[n] += r
    ...
    # computing raw rates
    # number of parameters, represented by 1 bit
    r = np.sum([v.size for v in wf]) + np.sum(v.size for v in bf)
...
def evaluate_qsg(num_workers, nn_params, qsg_params, seed):
    ...
    for n in range(max_iterations):
        ...
        for k in range(num_workers):
            ...
            rec_rate += (np.sum([v.size for v in sw]) + np.sum(v.size for v in sb))
            r = np.sum([cmp.compute_entropy(v) for v in qw]) + \
                np.sum([cmp.compute_entropy(v) for v in qb])
            entropy[n] += r
...
def test():
    ...
    ndqsg_params = {'ratio': 0.5, 'gradient-clip': None,
                    'num-levels': ((3), (3, 1)), 'bucket-size': bucket_size}
    ...
    for expr in [0]:  # range(3):
        ...
        if not os.path.exists(file_name):
            ...
            file_name = result_folder + 'base_model.mat'
            sio.savemat(file_name, mdict={'acc': base_accuracy,
                                          'rate': base_rate, 'w': w_base, 'b': b_base})
        else:
        ...
        for number_workers in [2, 4, 8, 16]:
            ...
            sio.savemat(file_name, mdict={'acc': accuracy,
                                          'rate': raw_rate, 'H': entropy, 'w': w, 'b': b})

            print('testing QSG method with 1 level, no bucket...')
            qsg_params = {'num-levels': 1, 'bucket-size': None}
            accuracy, entropy, raw_rate, w, b = evaluate_qsg(
                number_workers, nn_params, qsg_params, seed)
            ...
            ndqsg_params = {'ratio': 0.5, 'gradient-clip': None,
                            'num-levels': ((3), (3, 1)), 'bucket-size': None}
            accuracy, entropy, raw_rate, w, b = evaluate_ndqsg(
                number_workers, nn_params, ndqsg_params, seed)
